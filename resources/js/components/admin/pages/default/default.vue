...
export default {
    ...
    mounted() {
        fetch("/api/menuitems")
            ...
            .then(data => (this.menu_items = data.data))
    },
    computed: {
        total() {
            return this.orderItems.map(item => item.amount * item.price).reduce((a, b) => a + b, 0);
        }
    },
    methods: {
        category_item(category) {
            return menu_items.filter(item => item.category_id === category);
        },
